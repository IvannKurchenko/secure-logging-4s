...
class SecureLogger[F[_]](val underlying: Logger[F]) extends AnyVal {
  ...
  def error(t: Throwable)(message: => LogSecureString): F[Unit] = underlying.error(t)(message.value)
  ...
  def debug(t: Throwable)(message: => LogSecureString): F[Unit] = underlying.debug(t)(message.value)

  def trace(t: Throwable)(message: => LogSecureString): F[Unit] = underlying.trace(t)(message.value)
}
