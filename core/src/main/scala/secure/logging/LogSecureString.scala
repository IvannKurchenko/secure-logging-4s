 ...
 * Constructor is private, so it can be created only from within the library.
 * Can be created only from [[LogSecureString]] interpolation, which on it's turn expects
